...
fn beginDirectWriteStream(unit: c_int, rec: c_int, record_size: c_int) ?*DirectStreamState {
    ...
    const record = col6forge_direct_record_ptr(unit, rec, recl_i32) orelse return allocDirectStreamState(unit, rec, null, 0, 1);
    var i: usize = 0;
...
fn beginDirectReadStream(unit: c_int, rec: c_int, record_size_hint: c_int) ?*DirectStreamState {
    ...
    const record_ro = col6forge_direct_record_ptr_ro(unit, rec, recl_i32) orelse return allocDirectStreamState(unit, rec, null, 0, 1);
...
pub export fn col6forge_direct_write_stream_finish(state_any: ?*anyopaque) callconv(.c) c_int {
    ...
    if (stream.record != null) col6forge_direct_record_commit(stream.unit, stream.rec);
    return stream.status;
}
